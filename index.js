...
// ---------- APP SETUP (runs in both local & Vercel) ----------
...
// ---------- DB CONNECT + SEED (runs on cold start) ----------

(async () => {
  try {
    await connectDb(CONNECTION_STRING);
    await seedFromDatabase("./Kambaz/Database");
    console.log("MongoDB connected and database seeded");
  } catch (err) {
...
// When running locally (node index.js), process.env.VERCEL is undefined
...
// ---------- EXPORT FOR VERCEL (THIS FIXES YOUR ERROR) ----------
